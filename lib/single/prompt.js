...
module.exports = function ask(q, extra) {

	if (!q.textColor) q.textColor = {		question: "white",
			...
			if (inp.join("").length === 0 || !q.print && !q.censor) {
				...
				for (let option of q.options) {
					...
					qop.push({
						...
						short: option.length > inp.join("").length ? option.slice(0, -(option.length - inp.join("").length)) : option,
				...
				for (let option of qop) {
					...
					if (current.score < score && score > 0.49) current = {
					...
					if (q.autoCorrect === true || typeof q.autoCorrect === "number") {
						...
						for (let option of q.options) {
							...
							qop.push({
								...
								short: option.length > input.join("").length ? option.slice(0, -(option.length - input.join("").length)) : option,
						...
						for (let option of qop) {
							...
							if (current.score < score && score > (typeof q.autoCorrect === "number" ? q.autoCorrect : 0.69)) current = {
